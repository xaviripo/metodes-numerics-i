...
/*
 * File:   matrix_triagonal.h
 * Author: asolaroc7.alumnes
 *
 * Created on 9 / de novembre / 2016, 16:29
 */

#ifndef MATRIX_TRIAGONAL_H
#define MATRIX_TRIAGONAL_H
...
int palu_d3(int n, double *a, double *b, double *c, double tol) {
	...
	for (k = 0; k < n - 1; k++) {
		if (fabs(c[k+1]) < tol) {
                    return 1; /* si se intenta dividir
