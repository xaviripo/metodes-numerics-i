...
void exercici1();
int newton(int n,double* c,double* x, int iteracions, double tolerancia);
...
/*PART 1 CREEM EL POLINOMI*/
    ...
    if (newton(n, polinomi, listaZ, 8, 1e-10)){
        for(int l = 0; l < n;l++){
            printf("\nUna arrel és: %25.16lf",listaZ[l]);
        }
 ...
 */
int newton(int n,double* polinomi,double* listaZ, int iteracions, double tolerancia){
    ...
    for(int k = 0;k<n;k++){
        ...
        if(abs(x_ant - x_next) < tolerancia) listaZ[k] =  x_next;
        else return 0;
    }
    return 1;
}
